...
import { getGearCategory } from "../store/gear/gear.fetch";
...
const GearCategory = () => {
  ...
  const { gearCategories } = useSelector((state) => state.gear);
          ...
          didOpen: async () => {
            ...
            try {
              const { data } = await AxiosRequest.post(
                "/admin/gearCategory/deleteByIds",
                { gear_ids: deleteIds }
              );
  ...
  useEffect(() => {
    ...
    dispatch(getGearCategory(payload));
  }, [filter, dispatch, sort]);

  const selectAll = (checked) => {
    if (checked) {
      const selectedIds = gearCategories.records.map((r) => r._id);
      setIds(selectedIds);
    } else {
      setIds([]);
    }
          ...
          <div className="mediaLiabraryContent">
                  ...
                  <thead>
                    <tr>
                      <th className="bgDark">
                        <Form.Check
                          checked={
                            gearCategories.records.length &&
                            ids.length === gearCategories.records.length
                          }
                          onChange={(e) => selectAll(e.target.checked)}
                        />
                      </th>
                      <th className="bgDark">Name</th>
                      {/* <th className="bgDark">Number Of Product</th> */}
                      <th className="bgDark">Status</th>
                      <th className="bgDark">Id</th>
                      <th className="bgDark">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {gearCategories.records.map((category, index) => (
                      <tr key={index}>
                        <td>
                          <Form.Check
                            checked={ids.includes(category._id)}
                            onChange={(e) =>
                              selectOne(e.target.checked, category._id)
                            }
                          />
                        </td>
                        <td>{category.name}</td>
            ...
            <div className="paginationMain w-100 mb-4">
              <div className="pageEntries">
                Showing {gearCategories.from} to {gearCategories.to} of{" "}
                {gearCategories.total} entries
              </div>
              <div className="d-flex justify-content-center paginationCustom">
                <ReactPaginate
                  ...
                  pageCount={gearCategories.pages}
