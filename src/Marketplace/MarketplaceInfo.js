...
import { createTrade } from "../store/trade/trade.fetch";
...
import { getImageByTier, getTierName } from "../utils/tier";

const MarketplaceInfo = ({ setNft, nft }) => {
  const { id } = useParams();
  const [nftSelected, setNftSelected] = useState(false);
  const [blockedNft, setBlockedNft] = useState(null);
  const [ethPrice, setEthPrice] = useState(0);
  const [isAlreadyPurchase] = useState(false);
  const { user } = useSelector((_state) => _state.auth);
  const { nfts, nftsTiers } = useSelector((_state) => _state.nft);
  const collectionInfo = useSelector(
    (_state) => _state.collection.collectionInfo
  );

  const _nftsTiers = useMemo(() => {
    const tiers = [];

    const rookie = nftsTiers.records.find((nft) => nft.tier === "rookie");
    if (rookie) tiers.push(rookie);

    const vet = nftsTiers.records.find(
      (nft) => nft.tier === "vet" || nft.tier === "veteran"
    );
    if (vet) tiers.push(vet);

    const hof = nftsTiers.records.find((nft) => nft.tier === "hof");
    if (hof) tiers.push(hof);

    const goat = nftsTiers.records.find((nft) => nft.tier === "goat");
    if (goat) tiers.push(goat);
  ...
  useEffect(() => {
    getEthPrice().then((response) => setEthPrice(response.usd));
  }, []);

  useEffect(() => {
    if (nftsTiers.records.length && nftSelected === false) {
      setNft(nftsTiers.records[0]);
      setNftSelected(true);
    }
  ...
  const handleBuyNow = () => {
    if (!user) {
      navigate(`/login?redirect=/marketplace/${id}/info`, { replace: true });
    } else {
      blockNft({ playerId: nft?.playerId, tier: nft?.tier }).then((res) => {
        setBlockedNft(res);
  ...
  const buyNft = async (values) => {
    const blockedNftRes = await checkBlockedNft(blockedNft?._id);
    if (blockedNftRes) {
  ...
  const handleClose = () => {
    setModalShowPurchaseNFT(false);
    if (blockedNft) {
      unBlockedNftById(blockedNft?._id);
    }
  };
            ...
            <Col xxl={7} xl={12}>
              ...
              <div className="font32light" style={{ fontStyle: "normal" }}>
                ...
                <div className="seriesDetails">
                  ...
                  <div className="font32Yellow">{getTierName(nft.tier)}</div>
                </div>
              </div>
              ...
              <div className="mt-4 d-flex flex-wrap">
                {!isAlreadyPurchase ? (
                  ...
                  <div className="font32light" style={{ fontStyle: "normal" }}>
                    <Badge bg="danger">Already Bought</Badge>
                  </div>
                )}
...
function ModalPurchaseNFT({
  ...
  useEffect(() => {
    ...
    if (publickey) {
      ...
      getBalances(publickey).then((response) => setBalance(response));
    }
  }, []);
  ...
  const confirm = async () => {
    try {
      ...
      if (publicKey) {
        ...
        if (Number(l1Balance) <= Number(blockedNft.ethereumPrice)) {
          await showMagicLinkWallet();
        } else {
          // const checkTradeRes = await checkTrade({
          //   collectionId: blockedNft.collections,
          // });

          // if (checkTradeRes) {
          const transactionId = await createTradeInImx(
            blockedNft.orderId,
            blockedNft.ethereumPrice,
            publicKey
          );
          if (transactionId) {
            navigate("/MarketplaceInProcess", {
              state: {
                nft: {
                  ...blockedNft,
                  usdPrice: Number(blockedNft.ethereumPrice * ethprice).toFixed(
                    6
                  ),
                },
              },
            ...
            const data = {
              ...
              seller: blockedNft.owner,
              nftId: blockedNft._id,
              totalEth: Number(blockedNft.ethereumPrice),
              totalUsd: Number(
                Number(blockedNft.ethereumPrice * ethprice).toFixed(6)
              ),
              gasEth: 0,
              gasUsd: 0,
            ...
            <Row className="justify-content-between mb-2">
              ...
              <Col md={"auto"} className="col-6 font24Black">
                <span className="font22GrayNormal">
                  ${Number(nft.ethereumPrice * ethprice).toFixed(6)}
                </span>{" "}
